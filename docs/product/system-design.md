...
| 批准依据 | Hank 授权的一致性修正；独立系统设计复审 PASS；UAT 导航与公司页顺序于 2026-08-03 获批 |
...
完整产品保留首页、公司、项目、报告、统一搜索和“我的工作台”的信息架构，但导航按已交付产品域分阶段暴露。首个内部 UAT 的左侧栏严格只显示“首页”、“公司信息库”和“行业报告库”；项目、统一搜索和工作台在对应产品域通过门禁后才可加入。管理中心只在用户拥有至少一个管理能力时出现；隐藏入口不能替代服务端拒绝。
...
首个公司 UAT 的详情页默认顺序是：Banner、公司简介、项目分布地图、项目分布饼图、产量图、财务图、相关信息和新闻。每个模块只在存在已验证数据时渲染；未知不得伪造为零值，单个模块失败不得拖垮整页。后续完整公司域可在保留这些业务模块的基础上增加来源、历史、内部分析和授权操作。
