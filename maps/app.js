import {
  ...
  resolveOperator,
  searchProjects,
} from "./app-core.js?v=20260630-global";

const VERSION = "20260630-global";
...
const start = async () => {
  ...
  try {
    ...
    const [manifest, centersPayload] = await Promise.all([
      fetch("operators.json?v=20260630-global").then((response) => {
        if (!response.ok) throw new Error(`operators.json returned ${response.status}`);
        return response.json();
      }),
      fetch("data/country-centers.json?v=20260630-global").then((response) => {
