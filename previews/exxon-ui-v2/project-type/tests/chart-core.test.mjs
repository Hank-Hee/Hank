...
test("matches the approved ExxonMobil project-type structure", () => {
  const payloadPath = path.resolve(testDir, "../../../../maps/data/exxonmobil.json");
  const payload = JSON.parse(fs.readFileSync(payloadPath, "utf8"));
  const mix = buildProjectTypeMix(payload.projects);

  assert.deepEqual(mix.map(({ key, count }) => [key, count]), [
    ["Gas-Condensate field", 248],
    ["Oil field", 90],
    ["Gas field", 19],
    ["Mixed field", 16],
  ]);
  assert.equal(mix.reduce((sum, item) => sum + item.count, 0), 373);
});
