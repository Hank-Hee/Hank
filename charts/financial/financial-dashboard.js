...
const renderSourceLinks = (element, payload, config) => {
  ...
  const useShortLabels = window.matchMedia("(max-width: 560px)").matches;
  element.innerHTML = `Source: ${renderLinks(useShortLabels ? "footerShortLabel" : "footerLabel")}`;
  if (!useShortLabels && element.scrollWidth > element.clientWidth) element.innerHTML = `Source: ${renderLinks("footerShortLabel")}`;
};
...
const attachLegendToggles = (card, chart) => {
  card.querySelectorAll(".legend-item[data-series]").forEach((button) => {
    button.onclick = () => {
...
const renderComboChart = (payload, mode, card) => {
  const config = payload.dashboards[mode];
  ...
  renderSourceLinks(card.querySelector("[data-source-line]"), payload, config);
  card.querySelector("[data-update-date]").textContent = config.updateDate || `Update date: ${payload.company.updatedOn?.replaceAll("-", "/") || ""}`;
};

const renderCurrent = (payload) => {
  document.querySelectorAll(".chart-card[data-dashboard]").forEach((card) => renderComboChart(payload, card.dataset.dashboard, card));
  document.title = `${payload.company?.name || COMPANY} Financial Dashboard`;
  window.__financialDashboardDebug = { payload, version: VERSION };
};
